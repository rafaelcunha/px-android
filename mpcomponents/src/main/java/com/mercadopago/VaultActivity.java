...
import com.mercadopago.mpservices.model.PaymentMethodRow;
...
public class VaultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (validParameters()) {
            ...
            // Init MercadoPagoComponents object with public key
    ...
    protected boolean onVaultActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == MercadoPagoComponents.CUSTOMER_CARDS_REQUEST_CODE) {

            resolveCustomerCardsRequest(resultCode, data);

        } else if (requestCode == MercadoPagoComponents.PAYMENT_METHODS_REQUEST_CODE) {
        ...
        } else if (requestCode == MercadoPagoComponents.ISSUERS_REQUEST_CODE) {

            resolveIssuersRequest(resultCode, data);

        } else if (requestCode == MercadoPagoComponents.GUESSING_CARD_REQUEST_CODE) {

            resolveGuessingCardRequest(resultCode, data);
        }

        return (requestCode == MercadoPagoComponents.CUSTOMER_CARDS_REQUEST_CODE) ||
                (requestCode == MercadoPagoComponents.PAYMENT_METHODS_REQUEST_CODE) ||
                (requestCode == MercadoPagoComponents.INSTALLMENTS_REQUEST_CODE) ||
                (requestCode == MercadoPagoComponents.ISSUERS_REQUEST_CODE);
    }
    ...
    protected void getInstallmentsAsync() {
        ...
        if (bin.length() == MercadoPagoServices.BIN_LENGTH) {
            LayoutUtil.showProgressLayout(mActivity);
    ...
    protected String getSelectedPMBin() {
        ...
        if (mSelectedPaymentMethodRow != null) {
            ...
            return mCardToken.getCardNumber().substring(0, MercadoPagoServices.BIN_LENGTH);
        }
    }
